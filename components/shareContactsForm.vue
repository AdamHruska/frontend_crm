...
const delegateContacts = async (newSharedAuthorId) => {
	...
	try {
		await Promise.all(
			props.selected.map((contact) =>
				axios.put(
					`${config.public.apiUrl}contacts/${contact.id}/shared-author`,
					{
						shared_author: newSharedAuthorId,
					},
					{
						headers: {
							Authorization: `Bearer ${sessionStorage.getItem("token")}`,
		...
		console.error("Sharing contacts failed:", error);
		alert("Sharing contacts failed");
	} finally {
	...
	>
		...
		<div class="absolute inset-0 bg-gray-400 bg-opacity-50 backdrop-blur-sm">
			...
			>
				<button
					type="button"
					@click="emits('cancelShareContacts')"
