...
const handleSearch = async () => {
	...
	try {
		const response = await axios.get(
			`${config.public.apiUrl}search-users-admin`,
			{
				params: {
		...
		if (response.data && response.data.users) {
			...
			authStore.searchQueryAdmin = searchInput.value;
...
onMounted(() => {
	if (authStore.searchQueryAdmin) {
		searchInput.value = authStore.searchQueryAdmin;
		//handleSearch();
	}
});
</script>
