...
onMounted(async () => {
	...
	aktivita.value = props.item.activity;
	datum_cas.value = props.item.dueDate;
	console.log("skuska cas", datum_cas.value);
});

const updateActivity = async () => {
	event.preventDefault();
	await todosStore.updateTodo(props.item.id, {
...
const deleteActivity = async () => {
	event.preventDefault();
	await todosStore.deleteTodo(props.item.id);
	emit("cancelToDoActivity");
};
</script>
		...
		>
			...
			<div class="flex justify-center items-center mt-3 gap-6">
				...
				<button
					...
					class="text-white bg-red-500 hover:bg-red-600 focus:ring-4 focus:outline-none focus:ring-red-300 font-medium rounded-lg text-sm w-full sm:w-auto px-8 py-2.5 text-center dark:bg-red-500 dark:hover:bg-red-600 dark:focus:ring-red-800"
				>
					Vymazať
				</button>
			</div>
